...
import { UserPlus, X, Globe, MapPin, Palette } from 'lucide-react';

interface CountryDetailsProps {
  countryId: string;
  countryName: string;
  contacts: Contact[];
  onAddContact: (contactData: Omit<Contact, 'id' | 'createdAt'>) => void;
      ...
      {!isFormOpen ? (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            {/* Big Prominent Central "Add Friend" Button placed centrally at the top when list exists */}
            {countryContacts.length > 0 && (
              <div className="flex justify-center pb-2 border-b border-slate-100/60">
                <button
                  onClick={handleAddNewClick}
                  className="w-full max-w-sm py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-sm hover:shadow-md transition-all transform hover:-translate-y-0.2 cursor-pointer"
                >
            ...
            {countryContacts.length > 0 ? (
              <div className="flex flex-col gap-3">
                <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest font-sans mb-1">
                  Active Connections ({countryContacts.length} {countryContacts.length === 1 ? 'person' : 'people'})
                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center py-12">
                ...
                <h4 className="font-sans font-semibold text-slate-705 text-sm">No connections in this country</h4>
                <p className="text-[11px] text-slate-420 font-sans mt-1.5 max-w-[245px] mx-auto leading-relaxed">
                  Record contacts, friends, or tour guide details met during your trip to {countryName}!
                </p>
